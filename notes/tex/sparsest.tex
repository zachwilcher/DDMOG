...
\begin{proof}
    ...
    Specifically, let \(\{(a_i, b_i) \mid i = 1,2,\cdots, 4x\}\) be an order \(4x\) skolem sequence.
    The set of triples \(\{(a_i - b_i, a_i, b_i)\}\) as remarked in \cite{skolemRemarksTripleSystems1958}
    consists of the elements \(1, 2, \cdots, 12x\) where
    each triple satisfies one of the equations above.
\end{proof}
